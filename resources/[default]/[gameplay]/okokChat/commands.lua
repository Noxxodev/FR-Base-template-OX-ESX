...
if Config.EnableUnicornCommand then
	RegisterCommand(Config.UnicornCommand, function(source, args, rawCommand)
		local xPlayer = ESX.GetPlayerFromId(source)
		local length = string.len(Config.UnicornCommand)
		local message = rawCommand:sub(length + 1)
		local time = os.date(Config.DateFormat)
		playerName = xPlayer.getName()
		local job = xPlayer.job.name

		if job == Config.UnicornJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message unicorn"><i class="fa-regular fa-martini-glass-citrus"></i> <b><span style="color: #D355F9">{0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
...
if Config.EnableBahamasCommand then
	RegisterCommand(Config.BahamasCommand, function(source, args, rawCommand)
		local xPlayer = ESX.GetPlayerFromId(source)
		local length = string.len(Config.BahamasCommand)
		local message = rawCommand:sub(length + 1)
		local time = os.date(Config.DateFormat)
		playerName = xPlayer.getName()
		local job = xPlayer.job.name

		if job == Config.BahamasJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message bahamas"><i class="fa-regular fa-martini-glass-citrus"></i> <b><span style="color: #55D4F9">{0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
...
if Config.EnableVigneronCommand then
	RegisterCommand(Config.VigneronCommand, function(source, args, rawCommand)
		local xPlayer = ESX.GetPlayerFromId(source)
		local length = string.len(Config.VigneronCommand)
		local message = rawCommand:sub(length + 1)
		local time = os.date(Config.DateFormat)
		playerName = xPlayer.getName()
		local job = xPlayer.job.name

		if job == Config.VigneronJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message vigneron"><i class="fa-regular fa-martini-glass-citrus"></i> <b><span style="color: #55D4F9">{0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
