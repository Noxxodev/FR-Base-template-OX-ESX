...
lib.callback.register('om_jobcentre:getJobs', function(source)
    return jobs
end)

RegisterNetEvent('om_jobcentre:hired', function(index, job)
    local source = source
...
lib.callback.register('om_jobcentre:buy', function(source, index, licenseIndex)
    if #(GetEntityCoords(GetPlayerPed(source)) - vector3(Config.Locations[index].Coords)) < 5.0 then
        ...
        if license.LicenseNeeded ~= false then
            ...
            if success then
                ...
                return false, "Permission insufisante"
            end
        else
            local xPlayer = ESX.GetPlayerFromId(source)
            if xPlayer.getAccount('money').money >= license.Price then
