...
if Config.DoubleJob.enable then
    ...
    ESX[Config.DoubleJob.does] = function(faction, gradef)
        gradef = tostring(gradef)
        if ESX[Config.DoubleJob.table][faction] and ESX[Config.DoubleJob.table][faction].grades[gradef] then
          return true
        end
        return false
    end
...
function ESX.GetNumPlayers(key, val)
    ...
    if type(val) == "table" then
       ...
       --if Config.DoubleJob.enable then
       -- if key == Config.DoubleJob.name then
       --     for _, v in ipairs(val) do
       --         numPlayers[v] = (ESX[Config.DoubleJob.jobcount][v] or 0)
       --     end
       --     return numPlayers
       -- end
       --end
    ...
    --if Config.DoubleJob.enable then
    --    if key == Config.DoubleJob.name then
    --        return (ESX[Config.DoubleJob.jobcount][val] or 0)
    --    end
    --end
