...
if Config.Multichar then
    AddEventHandler("esx:onPlayerJoined", function(src, char, data)
        ...
        while not next(ESX[Config.DoubleJob.table]) do
            Wait(50)
        end
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    -- xPlayer Creation
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, GetPlayerName(playerId), userData.coords, userData.metadata, Config.DoubleJob.enable == true and userData[Config.DoubleJob.name] or nil)
...
AddEventHandler("playerDropped", function(reason)
    ...
    if xPlayer then
        ...
        --local faction = xPlayer[Config.DoubleJob.get].name
        local currentJob = ESX.JobsPlayerCount[job]
        --local currentFaction = ESX[Config.DoubleJob.jobcount][faction]
        ESX.JobsPlayerCount[job] = ((currentJob and currentJob > 0) and currentJob or 1) - 1
        --ESX[Config.DoubleJob.jobcount][faction] = ((currentFaction and currentFaction > 0) and currentFaction or 1) - 1
        GlobalState[("%s:count"):format(job)] = ESX.JobsPlayerCount[job]
        --GlobalState[("%s:count"):format(faction)] = ESX[Config.DoubleJob.jobcount][faction]
...
AddEventHandler("esx:playerLoaded", function(_, xPlayer)
   ...
   -- ESX[Config.DoubleJob.jobcount][faction] = (ESX[Config.DoubleJob.jobcount][faction] or 0) + 1
    --GlobalState[factionKey] = ESX[Config.DoubleJob.jobcount][faction]
end)
...
--AddEventHandler(Config.DoubleJob.set, function(_, faction, lastFaction)
--    local lastFactionKey = ("%s:count"):format(lastFaction.name)
--    local factionKey = ("%s:count"):format(faction.name)
--    local currentLastFaction = ESX[Config.DoubleJob.jobcount][lastFaction.name]
--
--    ESX[Config.DoubleJob.jobcount][lastFaction.name] = ((currentLastFaction and currentLastFaction > 0) and currentLastFaction or 1) - 1
--    ESX[Config.DoubleJob.jobcount][faction.name] = (ESX[Config.DoubleJob.jobcount][faction.name] or 0) + 1
--
--    GlobalState[lastFactionKey] = ESX[Config.DoubleJob.jobcount][lastFaction.name]
--    GlobalState[factionKey] = ESX[Config.DoubleJob.jobcount][faction.name]
--end)
...
ESX.RegisterServerCallback("esx:getPlayerData", function(source, cb)
    ...
    cb({
        ...
        job = xPlayer.getJob(),
        [Config.DoubleJob.name] = Config.DoubleJob.enable and xPlayer[Config.DoubleJob.get] or nil,
