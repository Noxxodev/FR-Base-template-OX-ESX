...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata, faction)
    ...
    self[Config.DoubleJob.set] = function(faction, grade)
        grade = tostring(grade)
        local lastFaction = self[Config.DoubleJob.name]

        if not ESX[Config.DoubleJob.does](faction, grade) then
            return print(("[es_extended] [^3WARNING^7] Ignoring invalid ^5.%s()^7 usage for ID: ^5%s^7, ^5%s^7 : ^5%s^7"):format(Config.DoubleJob.set, self.source, Config.DoubleJob.name, faction))
        end

        local factionObject, gradeObject = ESX[Config.DoubleJob.table][faction], ESX[Config.DoubleJob.table][faction].grades[grade]

        self[Config.DoubleJob.name] = {
            id = factionObject.id,
